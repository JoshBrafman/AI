...
def inputMC(s):
    wins = {} #stores how many times MC won for each column
    ties = {} #stores how many times there was a tie after going in each column
    losses = {} #stores losses
    for i in range(0, columns): #initialize zero for all three dictionaries for every possible move
         wins[i] = 0
         ties[i] = 0
         losses[i] = 0
    for i in range(0, columns):
        for count in range(0, 100): #play 100 games for each column
            if s.board[0][i] != 0: #if moving to this column is illegal we can skip the iteration to speed up the time
                 continue
            tmp = cpy(s)
            makeMove(tmp, i) #make a move to column i
            legalColumns = [n for n in range(0,columns)] #initialize every column as legal
            for c in range(0, columns):
                 if tmp.board[0][c] != 0: #this column is not legal
                      legalColumns.remove(c)
            while not isFinished(tmp): #play a random game until the end
                flag = True
                while flag:
                    #c = random.randrange(0, columns)
                    c = random.choice(legalColumns) #speed up time by only picking from legal columns
                    if c < 0 or c >= columns or tmp.board[0][c] != 0: #and therefore this should never
                        #print("Illegal move.")
                        #printState(s)
                        x = 3 #do nothing
                        ...
                        flag = False
                        makeMove(tmp, c)
                        if tmp.board[0][c] != 0: #this column is no longer legal
                            legalColumns.remove(c)
            #the random game has now finished
            if value(tmp) == VICTORY: #MC won
                    wins[i] += 1
            if value(tmp) == TIE:
                    ties[i] += 1
            if value(tmp) == LOSS: #MC lost
                    losses[i] += 1
    mostWins = -1
    bestColumn = -1
    for i in range(0, columns): #find which column had the highest win rate
         if wins[i] > mostWins:
              mostWins = wins[i]
              bestColumn = i
    #print("moving to", bestColumn)
    makeMove(s, bestColumn)
